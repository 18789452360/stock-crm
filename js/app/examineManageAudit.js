 ...
 */
require(['vue', 'layui', 'common', 'ajaxurl', 'tools', 'layers', 'text!/assets/popup/examine-audit.html', 'text!/assets/popup/edit-remark.html', 'template', 'lightbox'], function (Vue, layui, common, ajaxurl, tool, layers, examineAudit, editRemark, template ,lightbox) {
    ...
    var main = {
        ...
        examinePage: function() {
            ...
            tool.ajax({
                url: ajaxurl.examine.examine_view,
                type: 'post',
                data: {cooper_id: selCode},
                success: function(result){
                    if(result.code == 1){
                        // 渲染到vue数据层
                        vm.examineList = result.data;
         ...
         */
        lightboxs: function(){
            var boxs = $('#financeCkeckBoxs').find('div[id^="sign-imgs-box"]');
            boxs.each(function(){
                var id = $(this).prop('id');
                lightbox.init('#'+id);
            });
