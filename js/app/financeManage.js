 ...
 */
require(['moment', 'layui', 'common', 'ajaxurl', 'tools', 'layers', 'text!/assets/popup/finance-audit.html', 'text!/assets/popup/export-financal.html'], function (moment, layui, common, ajaxurl, tool, layers, financeAudit, exportFinancal) {
    ...
    var main = {
        ...
        getWaitList: function(keywords) {
            ...
            // 待财务审核
            tool.ajax({
                url: ajaxurl.financial.financial_list,
                type: 'post',
                data: vm.dataWait,
                beforeSend: function() {
        ...
        getAllList: function(page, callback) {
            ...
            // 全部审核记录
            tool.ajax({
                url: ajaxurl.financial.financial_record_list,
                type: 'post',
                data: {
    ...
    var vm = new Vue({
        ...
        data: {
            ...
            conditionStr: ['财务状态', '移交时间'],
            conditionTime: ['今天', '昨天', '最近7天', '最近30天', '待处理', '审核通过', '审核拒绝'],
            condition: [
                {name: '财务状态', show: false, active: false},
                ...
                if (customer) {// 自定义
                    ...
                    switch ($(e.target).text()) {
                        ...
                        case vm.conditionTime[5]:// 审核通过
                            vm.contractConditon.status = 'cwytg';
                            break;
                        case vm.conditionTime[6]:// 审核拒绝
                            vm.contractConditon.status = 'cwyjj';
                            break;
                        default:
                    }
                    switch (index) {
